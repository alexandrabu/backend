...
const seedData = async () => {
  try {
    ...
    const user1 = await User.create({ name: 'Alice', email: 'alice@example.com', department_id: department1.id });
    const user2 = await User.create({ name: 'Bob', email: 'bob@example.com', department_id: department2.id });
    const user3 = await User.create({ name: 'Charlie', email: 'charlie@example.com', department_id: department3.id });
    const user4 = await User.create({ name: 'David', email: 'david@example.com', department_id: department4.id });
    const user5 = await User.create({ name: 'Emma', email: 'emma@example.com', department_id: department1.id });
    const user6 = await User.create({ name: 'Frank', email: 'frank@example.com', department_id: department2.id });
